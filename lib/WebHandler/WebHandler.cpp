...
*/
        ...
        ESP.getFlashChipSpeed(), APP_NAME, APP_VERSION, __DATE__, __TIME__,

#ifdef HAVE_ETH_IF
        ETH.localIP().toString().c_str(), ETH.gatewayIP().toString().c_str(),
        ETH.subnetMask().toString().c_str(), ETH.dnsIP().toString().c_str(),
        ...
        /*
                  ...
                  WiFi.dnsIP().toString().c_str(),
        */
        fsTotalBytes, fsUsedBytes, ESP.getFreeHeap(), ESP.getSketchSize(),
        ESP.getFreeSketchSpace(), server.client().remoteIP().toString().c_str(),
